...
//		private GridNode GoalNode;
		...
		public enum DrawStatus{normal,forest,rock,blocked,BK,RK,GK,BD,RD,GD,TS,SN}
...
//		public GridNode getStartNode{ get { return StartNode;} }
		...
		void ButtonSwitch(){
			...
			case DrawStatus.SN:
				ui.Reset ();
				ui.SN.color = ui.selected_color;
				ui.SN.transform.localScale = new Vector3 (1.8f, 1.8f, 1.8f);
				break;
			}
...
//			}
			...
			if (Input.GetKeyDown (KeyCode.Alpha5))
				SetDrawStatus (DrawStatus.TS);
			if (Input.GetKeyDown (KeyCode.Alpha6))
				SetDrawStatus (DrawStatus.SN);
...
//				SetDrawStatus (Grid.DrawStatus.goal);
			...
			if ( Input.GetMouseButton( 0 ) && MouseNode ) {
				...
				if (Input.GetMouseButton (0) && drawStatus == DrawStatus.SN) {
					treasureFind.DrawSN (MouseRC);
				}
            }
